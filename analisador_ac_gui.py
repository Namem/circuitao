...
class ACCircuitAnalyzerApp:
    ...
    def _get_single_frequency_analysis_details(self, circuit_params, specific_freq):
        ...
        try:
            ...
            elif r_val==0 and topology=="Paralelo": z_r_val = complex(1e-12,0)
            if l_val > 0 and freq > 0: xl_val=2*cmath.pi*freq*l_val; z_l_val=complex(0,xl_val)
            elif l_val==0: z_l_val=complex(0,0)
            ...
            if topology == "Série":
                ...
                y_l=1/z_l_val if abs(z_l_val)>1e-12 else (complex(float('inf'),0) if l_val>0 and freq>0 else complex(0,0) )
                if l_val==0 and freq > 0: y_l=complex(float('inf'),0) 
                elif l_val==0 and freq == 0: y_l=complex(float('inf'),0) 
            ...
            if is_circuit_open_or_no_current and not (topology=="Paralelo" and r_val==0 and abs(v_source_phasor)>1e-9):
                ...
                if abs(q_reactive)>1e-6 or abs(q_l_comp+q_c_comp)>1e-6:
                    ...
                    output += f"    (Verificação Q_L+Q_C ≈ Q_total: {'Sim' if math.isclose(q_l_comp+q_c_comp, q_reactive, rel_tol=1e-2, abs_tol=abs_tol_q_sum) else 'Não'})\n"
            return output
        except Exception as e:
            import traceback; traceback.print_exc()
    ...
    def _update_embedded_plot(self, frequencies, plot_data_y, y_label_choice, f0_resonance=None, extremum_info=None):
        ...
        if extremum_info:
            ...
            self.ax_embedded.plot(efreq, evalue, marker='o', color=marker_color, markersize=6, fillstyle='none', markeredgewidth=1.2)
