...
class ACCircuitAnalyzerApp:
    ...
    def _validate_all_parameters(self, silent=True, check_detail_freq=False):
        ...
        if 'r_val' in params:
            ...
            if params['r_val'] == 0 and params['topology'] == "Paralelo" and not silent: 
    ...
    def _trigger_realtime_plot_update(self, event=None, from_combobox_value=None):
        ...
        if params:
            ...
            # print(f"Parâmetros inválidos para atualização em tempo real (trigger): {errors}") # Opcional: menos verboso no console
            self._clear_embedded_plot()
    ...
    def _find_extremum(self, frequencies, data_y, plot_choice, topology):
        ...
        if "|" in plot_choice: 
            ...
            try:
                original_indices = [i for i, val in enumerate(data_y) if math.isclose(val, extremum_value, rel_tol=1e-9)] # Usar isclose para floats
                if original_indices:
    ...
    def show_about_dialog_ctk(self):
        ...
        self.about_dialog_window.geometry("450x420") # Aumentar altura
        ...
        info_text = ("Versão: 1.7.0 (CustomTkinter)\n\n" 
        ...
        if popup_width<=1 or popup_height<=1: 
            ...
            except: popup_width,popup_height=450,420
    ...
    def analyze_circuit(self): 
        ...
        try:
            ...
            if f0_calc is not None:
                ...
                r_val = params.get('r_val', 0)
                l_val = params.get('l_val', 0)
                c_val = params.get('c_val', 0) # Adicionado para uso em Q paralelo, se necessário
                q_factor_val = None

                if params['topology'] == "Série":
                    if r_val > 1e-9 and l_val > 1e-9: q_factor_val = (omega_0 * l_val) / r_val
                    elif r_val > 1e-9 and c_val > 1e-9 and omega_0 > 1e-9 : q_factor_val = 1 / (omega_0 * c_val * r_val) # Usa C se L for zero mas C não
                    elif r_val < 1e-9 and l_val > 1e-9 and c_val > 1e-9: q_factor_val = float('inf') # R é zero
                
                elif params['topology'] == "Paralelo":
                    # Q para R || L || C ideal.
                    if l_val > 1e-9 and r_val > 1e-9 and omega_0 > 1e-9: q_factor_val = r_val / (omega_0 * l_val)
                    elif c_val > 1e-9 and r_val > 1e-9 and omega_0 > 1e-9 : q_factor_val = omega_0 * c_val * r_val # Usa C se L for zero
                    elif r_val < 1e-9 and (l_val > 1e-9 or c_val > 1e-9): q_factor_val = 0 # R é zero, Q é zero (curto)

                if q_factor_val is not None:
                    if q_factor_val == float('inf'):
                        q_factor_str = "Infinito"
                        bandwidth_str = "0.00 Hz"
                    elif q_factor_val > 1e-9:
                        q_factor_str = f"{q_factor_val:.2f}"
                        bandwidth_str = f"{f0_calc / q_factor_val:.2f} Hz"
                    else: # Q muito baixo ou zero
                        q_factor_str = f"{q_factor_val:.2f} (Baixo)"
                        bandwidth_str = "Muito Larga"
    ...
    def _get_single_frequency_analysis_details(self, circuit_params, specific_freq):
        # ... (Este método permanece o mesmo da versão anterior com potência por componente) ...
        output = ""
        try:
            ...
            if r_val == 0:
                z_r = complex(0,0) if topology == "Série" else complex(1e-12, 0) 
            else: z_r = complex(r_val, 0)
            xl = 0.0
            if l_val > 0 and freq > 0: xl = 2*cmath.pi*freq*l_val; z_l = complex(0, xl)
            elif l_val == 0: z_l = complex(0,0)
            else: z_l = complex(0,0) 
            xc = 0.0
            if c_val > 0 and freq > 0: xc = -1/(2*cmath.pi*freq*c_val); z_c = complex(0, xc)
            else: z_c = complex(float('inf'), 0)
            ...
            if topology == "Série":
                ...
                v_r_phasor=i_total_source_phasor*z_r; v_l_phasor=i_total_source_phasor*z_l
                ...
                y_r=1/z_r if abs(z_r)>1e-12 else complex(float('inf'),0) 
                y_l=1/z_l if abs(z_l)>1e-12 else (complex(float('inf'),0) if l_val>0 and freq>0 else complex(0,0) )
                if l_val==0: y_l=complex(float('inf'),0) 
                y_c=1/z_c if abs(z_c)>1e-12 else complex(0,0)
                if c_val==0 : y_c=complex(0,0) 
                y_total=y_r+y_l+y_c
                ...
                i_r_phasor=v_source_phasor*y_r if r_val>0 else (v_source_phasor/z_r if r_val==0 else complex(0,0))
                ...
                else: p_r_comp=abs(v_source_phasor*i_r_phasor.conjugate()).real 
